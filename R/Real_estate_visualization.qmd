...
map_data <- st_read("coords/TOWN_MOI_1140318.shp")
```
...
suppressMessages(
  suppressWarnings(
    invisible(capture.output(source("load_data.R")))
  )
)
...
```{r}
ggsave("hsinchu_map_large.png", plot = last_plot(), width = 13, height = 8, dpi = 300)
```
