...
if (file.exists(aging_path)) {
  ...
  if (!is.null(aging_data)) {
    message("✅ 高齡指數資料已載入：", nrow(aging_data), " 筆")
  }
} else {
  warning("⚠️ 找不到高齡指數資料：", aging_file)
}
...
for (type in names(medical_files)) {
  ...
  if (file.exists(file_path)) {
    ...
    if (!is.null(df)) {
      ...
      message("✅ 已載入醫療資料：", type, "（", nrow(df), " 筆）")
    }
  } else {
    warning("⚠️ 找不到醫療資料：", medical_files[[type]])
  }
}
...
for (region in region_list) {
  ...
  if (file.exists(geo_path)) {
    ...
    if (!is.null(df)) {
      geo_data[[region]] <- df
      message("✅ 已載入門牌座標：", region, "（", nrow(df), " 筆）")
    }
  } else {
    warning("⚠️ 找不到地區座標檔案：", geo_file)
  }
}
...
for (file in real_estate_files) {
  ...
  if (!is.null(df)) {
    ...
    message("✅ 已載入不動產資料：", basename(file), "（", nrow(df), " 筆）")
  }
}
...
# 🏠 Map Data
# ────────────────────────────────────────────────
map_files <- list.files(path_map, pattern = "\\.shp$", full.names = TRUE)
map_data <- list()

for (file in map_files) {
  df <- sf::st_read(file, quiet = TRUE)
  map_data[[basename(file)]] <- df
}
